...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw KnockInvalidDataException("`$name` is not set")
            is JsonNull -> throw KnockInvalidDataException("`$name` is null")
            else -> throw KnockInvalidDataException("`$name` is invalid, received $this")
        }
