...
/** List messages */
class ObjectListMessagesPageAsync private constructor(
    ...
    override fun toString() = "ObjectListMessagesPageAsync{objectsService=$objectsService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<ObjectListMessagesParams> {
      ...
      return Optional.of(ObjectListMessagesParams.builder().from(params).apply {pageInfo().flatMap { it.after()}.ifPresent{ this.after(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<ObjectListMessagesPageAsync>> {
      return getNextPageParams().map {
        objectsService.listMessages(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    companion object {
        ...
        @JvmStatic
        fun of(objectsService: ObjectServiceAsync, params: ObjectListMessagesParams, response: Response) =
    ...
    class AutoPager(
        private val firstPage: ObjectListMessagesPageAsync,

    ) {

        fun forEach(action: Predicate<Message>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ObjectListMessagesPageAsync>>.forEach(action: (Message) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
