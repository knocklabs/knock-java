...
/** List messages */
class MessageListPageAsync private constructor(
    ...
    override fun toString() = "MessageListPageAsync{messagesService=$messagesService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<MessageListParams> {
      ...
      return Optional.of(MessageListParams.builder().from(params).apply {pageInfo().flatMap { it.after()}.ifPresent{ this.after(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<MessageListPageAsync>> {
      return getNextPageParams().map {
        messagesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    companion object {
        ...
        @JvmStatic
        fun of(messagesService: MessageServiceAsync, params: MessageListParams, response: Response) =
    ...
    class AutoPager(
        private val firstPage: MessageListPageAsync,

    ) {

        fun forEach(action: Predicate<Message>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<MessageListPageAsync>>.forEach(action: (Message) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
