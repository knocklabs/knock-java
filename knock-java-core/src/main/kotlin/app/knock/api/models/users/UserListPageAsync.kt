...
/** List users */
class UserListPageAsync private constructor(
    ...
    fun entries(): List<User> = response().entries()
    ...
    fun getNextPageParams(): Optional<UserListParams> {
      ...
      return Optional.of(UserListParams.builder().from(params).apply {pageInfo().flatMap { it.after()}.ifPresent{ this.after(it) } }.build())
    }
    ...
    class Response @JsonCreator constructor(
        @JsonProperty("entries") private val entries: JsonField<List<User>> = JsonMissing.of(),
        @JsonProperty("page_info") private val pageInfo: JsonField<PageInfo> = JsonMissing.of(),
        @JsonAnySetter private val additionalProperties: Map<String, JsonValue> = immutableEmptyMap(),

    ) {

        fun entries(): List<User> = entries.getNullable("entries") ?: listOf()

        fun pageInfo(): Optional<PageInfo> = Optional.ofNullable(pageInfo.getNullable("page_info"))

        @JsonProperty("entries")
        fun _entries(): Optional<JsonField<List<User>>> = Optional.ofNullable(entries)
        ...
        class Builder {
            ...
            private var entries: JsonField<List<User>> = JsonMissing.of()
            ...
            fun entries(entries: JsonField<List<User>>) = apply { this.entries = entries }
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<User>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<UserListPageAsync>>.forEach(action: (User) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
