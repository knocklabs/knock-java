...
/** List schedules */
class UserListSchedulesPageAsync private constructor(
    ...
    override fun toString() = "UserListSchedulesPageAsync{usersService=$usersService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<UserListSchedulesParams> {
      ...
      return Optional.of(UserListSchedulesParams.builder().from(params).apply {pageInfo().flatMap { it.after()}.ifPresent{ this.after(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<UserListSchedulesPageAsync>> {
      return getNextPageParams().map {
        usersService.listSchedules(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    companion object {
        ...
        @JvmStatic
        fun of(usersService: UserServiceAsync, params: UserListSchedulesParams, response: Response) =
    ...
    class AutoPager(
        private val firstPage: UserListSchedulesPageAsync,

    ) {

        fun forEach(action: Predicate<Schedule>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<UserListSchedulesPageAsync>>.forEach(action: (Schedule) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
