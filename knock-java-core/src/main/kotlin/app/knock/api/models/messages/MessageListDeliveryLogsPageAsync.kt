...
/** List delivery logs */
class MessageListDeliveryLogsPageAsync private constructor(
    ...
    override fun toString() = "MessageListDeliveryLogsPageAsync{messagesService=$messagesService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<MessageListDeliveryLogsParams> {
      ...
      return Optional.of(MessageListDeliveryLogsParams.builder().from(params).apply {pageInfo().flatMap { it.after()}.ifPresent{ this.after(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<MessageListDeliveryLogsPageAsync>> {
      return getNextPageParams().map {
        messagesService.listDeliveryLogs(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    companion object {
        ...
        @JvmStatic
        fun of(messagesService: MessageServiceAsync, params: MessageListDeliveryLogsParams, response: Response) =
    ...
    class Response @JsonCreator constructor(
        @JsonProperty("entries") private val entries: JsonField<List<MessageDeliveryLog>> = JsonMissing.of(),
        ...
        class Builder {
            ...
            fun entries(entries: JsonField<List<MessageDeliveryLog>>) = apply { this.entries = entries }
    ...
    class AutoPager(
        private val firstPage: MessageListDeliveryLogsPageAsync,

    ) {

        fun forEach(action: Predicate<MessageDeliveryLog>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<MessageListDeliveryLogsPageAsync>>.forEach(action: (MessageDeliveryLog) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
